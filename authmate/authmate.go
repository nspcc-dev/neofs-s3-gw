...
func buildSessionTokenV2(key *keys.PrivateKey, lifetime lifetimeOptions, contexts []session2.Context, gateKeys []*keys.PublicKey, gatesNNS []string) ([]session2.Token, error) {
	var (
	...
	targets := make([]session2.Target, 0, len(gateKeys)+len(gatesNNS))
	for _, gateKey := range gateKeys {
		targets = append(targets, session2.NewTargetUser(user.NewFromScriptHash(gateKey.GetScriptHash())))
	}
	for _, nns := range gatesNNS {
		targets = append(targets, session2.NewTargetNamed(nns))
	}

	chunks := slices.Chunk(targets, session2.MaxSubjectsPerToken)
	...
	for keys := range chunks {
		...
		if err := tokenV2.SetSubjects(keys); err != nil {
...
func buildSessionTokensV2(key *keys.PrivateKey, lifetime lifetimeOptions, ctxs []sessionTokenContext, gatesKeys []*keys.PublicKey, gatesNNS []string) ([]session2.Token, error) {
	var (
	...
	sessionTokens, err := buildSessionTokenV2(key, lifetime, deduplicate, gatesKeys, gatesNNS)
	if err != nil {
...
func createTokens(options *IssueSecretOptions, lifetime lifetimeOptions) ([]*accessbox.GateData, error) {
	...
	if !options.SkipSessionRules {
		...
		sessionTokensV2, err := buildSessionTokensV2(options.NeoFSKey, lifetime, sessionRules, options.GatesPublicKeys, options.GatesNNS)
		if err != nil {
