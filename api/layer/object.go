...
func (n *layer) comprehensiveSearchAllVersionsInNeoFS(ctx context.Context, bkt *data.BucketInfo, owner user.ID, objectName string, onlyUnversioned bool) ([]allVersionsSearchResult, oid.ID, *data.LockInfo, error) {
	...
	var (
		...
		locksByVesions = make(map[string][]locksSearchResult)
	...
	for _, item := range searchResultItems {
		...
		case s3headers.TypeLock:
			...
			if item.Attributes[7] != "" {
				...
				if err = json.Unmarshal([]byte(item.Attributes[7]), &fields); err != nil {
					return nil, oid.ID{}, nil, fmt.Errorf("unmarshal lock meta fields: %w", err)
				}
				...
				if fields[s3headers.FieldRetentionUntilMode] != "" {
					...
					if err != nil {
						return nil, oid.ID{}, nil, fmt.Errorf("invalid retention until time: %w", err)
					}
			...
			if _, ok := locksByVesions[version]; !ok {
				locksByVesions[version] = make([]locksSearchResult, 1)
			}

			locksByVesions[version] = append(locksByVesions[version], lsr)
			continue
		default:
		}
	...
	if len(searchResults) > 0 {
		...
		if searchResults[0].IsVersioned {
			...
			lockVersions = locksByVesions[ver]
		} else {
			// empty version
			tagsObjectOID = tagsByVersion[""]
			lockVersions = locksByVesions[""]
		}
	...
	for _, item := range searchResultItems {
		...
		case s3headers.TypeLock:
			...
			if item.Attributes[4] != "" {
				fields := make(map[string]string)
				if err = json.Unmarshal([]byte(item.Attributes[4]), &fields); err != nil {
					return oid.ID{}, nil, fmt.Errorf("unmarshal retention fields: %w", err)
				}
				...
				if fields[s3headers.FieldRetentionUntilMode] != "" {
					ts, err := strconv.ParseInt(fields[s3headers.FieldRetentionUntilMode], 10, 64)
					if err != nil {
						return oid.ID{}, nil, fmt.Errorf("invalid retention until time: %w", err)
					}
