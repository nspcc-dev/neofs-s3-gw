...
func (n *layer) PutObject(ctx context.Context, p *PutObjectParams) (*data.ExtendedObjectInfo, error) {
	...
	prm.Attributes = make([][2]string, 0, len(p.Header)+1)

	if bktSettings.Unversioned() {
		prm.Attributes = append(prm.Attributes, [2]string{attrS3VersioningState, data.VersioningUnversioned})
	} else if bktSettings.VersioningSuspended() {
		prm.Attributes = append(prm.Attributes, [2]string{attrS3VersioningState, data.VersioningUnversioned})
	} else if bktSettings.VersioningEnabled() {
		prm.Attributes = append(prm.Attributes, [2]string{attrS3VersioningState, data.VersioningEnabled})
	}

	// if bktSettings.Unversioned() || bktSettings.VersioningSuspended() {
	// 	prm.Attributes = append(prm.Attributes, [2]string{attrS3VersioningState, "false"})
	// } else if bktSettings.VersioningEnabled() {
	// 	prm.Attributes = append(prm.Attributes, [2]string{attrS3VersioningState, "true"})
	// }
...
func (n *layer) searchAllVersionsInNeoFS(ctx context.Context, bkt *data.BucketInfo, owner user.ID, objectName string, onlyUnversioned bool) ([]*object.Object, error) {
	...
	if onlyUnversioned {
		prmSearch.Filters.AddFilter(attrS3VersioningState, data.VersioningUnversioned, object.MatchStringEqual)
	}
...
func (n *layer) getLatestObjectsVersions(ctx context.Context, p allObjectParams) (objects []*data.ObjectInfo, next *data.ObjectInfo, err error) {
	...
	if nodeVersions == nil {
		...
		// nodeVersions, err = convert(heads)
		// if err != nil {
		// 	return nil, nil, fmt.Errorf("convert nodes: %w", err)
		// }
...
func (n *layer) getAllObjectsVersions(ctx context.Context, bkt *data.BucketInfo, prefix, delimiter string) (map[string][]*data.ExtendedObjectInfo, error) {
	...
	for _, nodeVersion := range nodeVersions {
		...
		if isDeleteMarkerObject(*nodeVersion) {
			...
			oi = objectInfoFromMeta(bkt, nodeVersion)
...
func convert(head object.Object) data.NodeVersion {
	// var result = make([]*data.NodeVersion, 0, len(objs))
	nv := data.NodeVersion{
		BaseNodeVersion: data.BaseNodeVersion{
			OID: head.GetID(),
		},
