...
func (h *MultipartHashes) WritePayload(bts []byte) error {
	if h.Hash != nil {
		if _, err := h.Hash.Write(bts); err != nil {
			return fmt.Errorf("write hash: %w", err)
		}
	}

	if h.HomoHash != nil {
		if _, err := h.HomoHash.Write(bts); err != nil {
			return fmt.Errorf("write homo hash: %w", err)
		}
	}

	if h.PartHash != nil {
		if _, err := h.PartHash.Write(bts); err != nil {
			return fmt.Errorf("write part hash: %w", err)
		}
	}
