...
func (n *layer) getLockDataFromObjects(ctx context.Context, bkt *data.BucketInfo, objectName, version string) (*data.LockInfo, error) {
	...
	if version != "" {
		// var verOID oid.ID
		// if err := verOID.DecodeString(version); err != nil {
...
func (n *layer) putLockObject(ctx context.Context, bktInfo *data.BucketInfo, objID oid.ID, lock *data.ObjectLock, copiesNumber uint32, objectName, objectVersion string) (oid.ID, error) {
	...
	if objectVersion != "" {
		...
		prm.Attributes = append(prm.Attributes, [2]string{AttributeObjectVersion, objectVersion})
	}
...
func (n *layer) attributesFromLock(ctx context.Context, lock *data.ObjectLock) ([][2]string, error) {
	...
	if lock.Retention != nil {
		...
		result = append(result, [2]string{AttributeRetentionUntilMode, lock.Retention.Until.UTC().Format(time.RFC3339)})
