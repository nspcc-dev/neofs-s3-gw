...
func (n *layer) reUploadSegmentedPart(ctx context.Context, uploadParams UploadPartParams, part *data.PartInfo, bktInfo *data.BucketInfo, multipartInfo *data.MultipartInfo) error {
	...
	eg.Go(func() error {
		...
		for _, element := range part.Elements {
			...
			if err != nil {
				err = fmt.Errorf("get part oid=%s, emelent oid=%s: %w", part.OID.String(), element.OID.String(), err)
				break
			}

			if _, err = pipeWriter.Write(elementObj.Payload()); err != nil {
				err = fmt.Errorf("write part oid=%s, emelent oid=%s: %w", part.OID.String(), element.OID.String(), err)
				break
			}
		...
		if err != nil {
			...
			return fmt.Errorf("pipe: %w. PipeClose: %w", err, pipeCloseErr)
		}
