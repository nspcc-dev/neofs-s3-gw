...
func (c *center) parseAuthHeader(header, amzContent string) (*authHeader, error) {
...
func (c *center) checkSign(authHeader *authHeader, box *accessbox.Box, request *http.Request, signatureDateTime time.Time) error {
	credProvider := aws.NewCredentialsCache(credentials.NewStaticCredentialsProvider(authHeader.AccessKeyID, box.Gate.AccessKey, ""))
