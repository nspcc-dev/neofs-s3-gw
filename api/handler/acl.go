...
func (h *handler) authTokenIssuer(ctx context.Context) (user.ID, error) {
	...
	if box.Gate.SessionTokenV2 == nil {
		return user.ID{}, errors.New("missing session v2 token")
	}
...
func (h *handler) PutBucketACLHandler(w http.ResponseWriter, r *http.Request) {
	...
	if _, err = h.updateBucketACL(r, astBucket, bktInfo, tokenv2); err != nil {
...
func (h *handler) updateBucketACL(r *http.Request, astChild *ast, bktInfo *data.BucketInfo, sessionTokenV2 *session2.Token) (bool, error) {
	bucketACL, err := h.obj.GetBucketACL(r.Context(), bktInfo)
	if err != nil {
...
func (h *handler) PutBucketPolicyHandler(w http.ResponseWriter, r *http.Request) {
	...
	if _, err = h.updateBucketACL(r, astPolicy, bktInfo, tokenv2); err != nil {
		h.logAndSendError(w, "could not update bucket acl", reqInfo, err)
		return
	}
}
