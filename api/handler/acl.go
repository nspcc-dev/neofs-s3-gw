...
func parseACLHeaders(header http.Header, key *keys.PublicKey) (*AccessControlPolicy, error) {
	var (
		err     error
		account = user.NewFromScriptHash(key.GetScriptHash()).String()
	)
	...
	if found != nil {
		if !groupTarget {
			for _, key := range target.BinaryKeys() {
				pk, err := keys.NewPublicKeyFromString(hex.EncodeToString(key))
				if err != nil {
					panic(errInvalidPublicKey)
				}

				var exist bool
				var acc = user.NewFromScriptHash(pk.GetScriptHash())
		...
		if !groupTarget {
			for _, key := range target.BinaryKeys() {
				...
				astOperation.Users = append(astOperation.Users, user.NewFromScriptHash(pk.GetScriptHash()))
			}
...
func encodeObjectACL(log *zap.Logger, bucketACL *layer.BucketACL, bucketName, objectVersion string) *AccessControlPolicy {
	account := user.NewFromScriptHash(bucketACL.Info.OwnerPublicKey.GetScriptHash()).String()

	ownerGrantee := NewGrantee(granteeCanonicalUser)
	ownerGrantee.ID = account
